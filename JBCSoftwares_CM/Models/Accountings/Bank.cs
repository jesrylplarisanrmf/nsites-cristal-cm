...
namespace JCSoftwares_CM.Models.Accountings
{
    public class Bank
    {
        ...
        public DataTable getBanks(string pDisplayType, string pPrimaryKey, string pSearchString)
        {
            ...
            {
                _conn.Open();
                MySqlDataAdapter _da = new MySqlDataAdapter("call spGetBanks('" + pDisplayType + "'," + (pPrimaryKey == null ? "0" : pPrimaryKey) + ",'" + pSearchString + "');", _conn);
                _da.Fill(_dt);
                _conn.Close();
        ...
        public string insertBank(Bank pBank)
        {
            string _Id = "";
            using (MySqlConnection _conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["MySqlConnection"].ConnectionString))
            {
                ...
                MySqlCommand _cmd = new MySqlCommand("call spInsertBank('" + pBank.Code +
                    "','" + pBank.Description +
                    "','" + pBank.AccountNo +
                    "','" + pBank.EmailAddress +
                    "','" + pBank.ContactPerson +
        ...
        public string updateBank(Bank pBank)
        {
            string _Id = "";
            using (MySqlConnection _conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["MySqlConnection"].ConnectionString))
            {
                _conn.Open();
                MySqlTransaction _trans = _conn.BeginTransaction();
                MySqlCommand _cmd = new MySqlCommand("call spUpdateBank('" + pBank.Id +
                    "','" + pBank.Code +
                    "','" + pBank.Description +
                    "','" + pBank.AccountNo +
                    "','" + pBank.EmailAddress +
        ...
        public bool removeBank(string pId, string pUserId)
        {
            ...
            using (MySqlConnection _conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["MySqlConnection"].ConnectionString))
            {
                _conn.Open();
                MySqlTransaction _trans = _conn.BeginTransaction();
                MySqlCommand _cmd = new MySqlCommand("call spRemoveBank('" + pId +
                    "','" + pUserId + "');", _conn);
                try
                {
                    _cmd.Transaction = _trans;
